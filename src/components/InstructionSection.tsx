...
// 核心自我清理機制：自動偵測並從工作表中徹底刪除「甜度」與「冰塊」兩個舊欄位以對齊資料結構，並自動補足缺失的聯絡及送貨日期欄位
function cleanSheetHeaders(sheet) {
  ...
  // 4. 自動補齊缺失的聯絡人/電子信箱/地址/配送日期欄位 (非常重要！防止老舊工作表遺漏欄位導致讀取為空)
  var requiredHeaders = ["手機號碼", "電子信箱", "送貨地址", "送貨日期"];
  var headersUpdated = false;
  
  for (var k = 0; k < requiredHeaders.length; k++) {
    var reqHeader = requiredHeaders[k];
    if (headersLine.indexOf(reqHeader) === -1) {
      var nextColIdx = headersLine.length + 1;
      sheet.getRange(1, nextColIdx).setValue(reqHeader);
      headersLine.push(reqHeader);
      headersUpdated = true;
    }
  }

  if (headersUpdated) {
    sheet.getRange(1, 1, 1, headersLine.length)
         .setFontWeight("bold")
...
function doGet() {
  ...
  if (orderSheet) {
    ...
    if (orderData.length > 1) {
      ...
      var phoneIdx = getIndex(["手機號碼", "手機"], 6);
      var emailIdx = getIndex(["電子信箱", "email", "e-mail", "電子郵件"], 7);
      var addressIdx = getIndex(["送貨地址", "收件地址", "地址"], 8);
      var deliveryDateIdx = getIndex(["送貨日期", "期望送貨日期", "送貨日"], 9);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm hover:shadow-md p-6 border border-natural-border mb-8 transition-all duration-300">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="serif text-lg font-bold text-[#2C2C2C]">雲端試算表串接指南 (Google Apps Script 後端設定)</h2>
        ...
        >
          {isOpen ? '收合雲端試算表串接指南' : '展開雲端試算表串接指南'}
